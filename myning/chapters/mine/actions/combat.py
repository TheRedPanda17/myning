...
from myning.chapters.mine.actions import Action
from myning.chapters.mine.combat_minigame import CombatMinigame
...
from myning.utilities.generators import generate_enemy_army, generate_reward
...
class CombatAction(Action):
    ...
    @property
    def content(self):
        if not settings.mini_games_disabled:
            return self.game
        allies = Army(player.army.living_members)
        enemies = Army(self.enemies.living_members)
        table = Table.grid()
        table.add_row("[orange1]Oh no! You're under attack[/]\n")
        table.add_row(f"[bold]Round {self.round}[/]")
        table.add_row(f"Fighting... ({self.duration} seconds left)\n")
        table.add_row("⚔️   " * (5 - (self.duration - 1) % 5))
        table.add_row("\n[bold]Your Army[/]")
    ...
    def fight(self, bonus: float):
        ...
        for attacker in battle_order:
            ...
            if is_friendly:
                damage, dodged, crit = _calculate_damage(attacker, defender, bonus)
            else:
...
class RoundAction(Action):
    def __init__(
        ...
        damage_width = max(len(str(damage_done)), len(str(health_lost)))
        self.summary = "[bold]Round Summary[/]\n\n"
        if not settings.mini_games_disabled:
            self.summary += f"{_bonus_str(bonus)}\n\n"
        self.summary += (
            f"[bold green1]{damage_done:{damage_width}}[/] damage inflicted.\n"
...
def _calculate_damage(attacker: Character, defender: Character, bonus: float = 1):
...
def _bonus_str(bonus: float):
    return (
        "[bold]No[/] damage bonus"
        if bonus == 1
        else f"[bold green1]{Formatter.percentage(bonus)}[/] damage bonus"
        if bonus > 1
        else f"[bold red1]{Formatter.percentage(bonus)}[/] damage penalty"
    )
